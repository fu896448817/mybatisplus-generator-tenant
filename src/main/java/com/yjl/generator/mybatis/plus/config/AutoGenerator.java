 ...
 * the License.
 */
package com.yjl.generator.mybatis.plus.config;
...
public class AutoGenerator {
    ...
    private static final Log logger =
            LogFactory.getLog(com.baomidou.mybatisplus.generator.AutoGenerator.class);
    ...
    private static String wordsAndHyphenAndCamelToConstantCase(String input) {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            if ((lastOneIsNotUnderscore) && ((isUpperCaseAndPreviousIsLowerCase)
                    || (previousIsWhitespace) || ((_betweenUpperCases) && (containsLowerCase)
                            && (isUpperCaseAndPreviousIsUpperCase)))) {
                buf.append("_");
            } else if (((separatorAfterDigit) && (Character.isDigit(previousChar))
                    && (Character.isLetter(c)))
                    || ((separatorBeforeDigit) && (Character.isDigit(c))
            ...
            if ((shouldReplace(c)) && (lastOneIsNotUnderscore)) {
                buf.append('_');
                    ...
                    && ((isNotUnderscore) || (lastOneIsNotUnderscore))) {
    ...
    private void batchOutput(String entityName, VelocityContext context) {
        try {
            ...
            String entityFile =
                    String.format((pathInfo.get(ConstVal.ENTITY_PATH) + ConstVal.ENTITY_NAME),
                            entityName + "Entity");
            String mapperFile =
                    String.format(
