...
public class ConfigBuilder {
    ...
    private void handlerPackage(TemplateConfig template, String outputDir, PackageConfig config) {
        ...
        packageInfo.put(ConstVal.SERVICEIMPL,
        ...
        if (StringUtils.isNotEmpty(template.getEntity())) {
            pathInfo.put(ConstVal.ENTITY_PATH,
                    joinPath(outputDir, packageInfo.get(ConstVal.ENTITY)));
        }
        if (StringUtils.isNotEmpty(template.getMapper())) {
            pathInfo.put(ConstVal.MAPPER_PATH,
                    joinPath(outputDir, packageInfo.get(ConstVal.MAPPER)));
        }
    ...
    private List<TableInfo> processTable(List<TableInfo> tableList, NamingStrategy strategy,
        ...
        for (TableInfo tableInfo : tableList) {
            tableInfo.setEntityName(strategyConfig, NamingStrategy
                    .capitalFirst(processName(tableInfo.getName(), strategy, tablePrefix)));
            ...
            if (StringUtils.isNotEmpty(globalConfig.getXmlName())) {
                tableInfo.setXmlName(
                        String.format(globalConfig.getXmlName(), tableInfo.getEntityName()));
            } else {
    ...
    private TableInfo convertTableFields(TableInfo tableInfo, NamingStrategy strategy) {
        ...
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(
                    String.format(querySQL.getTableFieldsSql(), tableInfo.getName()));
            ResultSet results = preparedStatement.executeQuery();
            while (results.next()) {
                ...
                field.setColumnType(
                        dataSourceConfig.getTypeConvert().processTypeConvert(field.getType()));
